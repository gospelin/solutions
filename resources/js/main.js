...
function initialize() {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (typeof i18next === 'undefined') {
            console.error('i18next not loaded. Skipping translations.');
            return;
        }
        ...
        function loadTranslations(t) {
            fetch('/translations.json')
                ...
                .catch(error => {
                    console.warn('Translation fetch failed. Using defaults:', error);
                    applyTranslations(t);
                    setupDynamicContent(t);
                });
        }
        ...
        function setupDynamicContent(t) {
                ...
                if (modal && modalClose && modalIcon && modalTitle && modalDescription && serviceCards.length) {
                    serviceCards.forEach(card => {
                        ...
                        card.clickHandler = () => {
                            ...
                            gsap.fromTo(modal.querySelector('.modal-content'), { opacity: 0, y: 50 }, { opacity: 1, y: 0, duration: 0.5, ease: 'power3.out' });
                            modalClose.focus();
                        };
            ...
            if (mobileMenu && navLinks) {
                mobileMenu.addEventListener('click', () => {
                    ...
                    hamburgers[0].style.transform = navLinks.classList.contains('active') ? 'rotate(45deg) translate(5px, 5px)' : 'none';
                    hamburgers[1].style.opacity = navLinks.classList.contains('active') ? '0' : '1';
                    hamburgers[2].style.transform = navLinks.classList.contains('active') ? 'rotate(-45deg) translate(7px, -7px)' : 'none';
                });
            ...
            if (themeToggleButton && themeTray && themeOptions.length) {
                themeToggleButton.addEventListener('click', () => {
                    ...
                    gsap.to(themeTray, {
                        ...
                        y: isActive ? 10 : 0,
                        ease: 'power3.out',
                        onComplete: () => { if (isActive) themeTray.style.display = 'none'; },
                        onStart: () => { if (!isActive) themeTray.style.display = 'flex'; }
                    });
                });
            ...
            heroButtons.forEach(button => {
                button.addEventListener('mouseenter', () => {
                    gsap.to(button, { scale: 1.05, duration: 0.3, ease: 'power3.out', boxShadow: '0 8px 24px rgba(108, 92, 231, 0.5)' });
                });
                button.addEventListener('mouseleave', () => {
                    gsap.to(button, { scale: 1, duration: 0.3, ease: 'power2.in', boxShadow: 'none' });
                });
            });
            ...
            faqQuestions.forEach(question => {
                question.addEventListener('mouseenter', () => {
                    gsap.to(question, { backgroundColor: 'rgba(59, 130, 246, 0.1)', duration: 0.3, ease: 'power3.out' });
                });
                question.addEventListener('mouseleave', () => {
                    gsap.to(question, { backgroundColor: 'transparent', duration: 0.3, ease: 'power2.in' });
                });
            });
            ...
            gsap.utils.toArray('.floating-element').forEach(element => {
                ...
                gsap.to(element, {
                    y: '-=100',
                    ease: 'none',
                    scrollTrigger: { trigger: '.hero', start: 'top top', end: 'bottom top', scrub: speed }
                });
            });
