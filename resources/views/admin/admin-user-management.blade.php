...
@section('description', 'Manage platform admin accounts, including bans and deletions.')
    ...
    <section class="users-container">
        <div class="content-header">
            <div>
                ...
                <p class="page-subtitle">View and control admin accounts with ease.</p>
            </div>
        </div>
        ...
        <div class="chart-card">
            <div class="chart-header">
                <h3 class="chart-title">Admins</h3>
                <div class="chart-actions">
                    <input type="text" class="search-input" id="userSearch" placeholder="Search by name or email..." aria-label="Search admins">
            ...
            <div class="table-container">
                <table class="user-table" role="grid" aria-label="Admin management table">
                    <thead>
                        <tr>
                            <th scope="col">ID</th>
                                    ...
                                    <div class="action-group" role="group" aria-label="Actions for {{ $user->name }}">
                                        <form action="{{ route('admin.users.ban', $user->id) }}" method="POST" style="display: inline;">
                                            ...
                                            <button type="submit" class="action-btn {{ ($user->status ?? 'Active') == 'Banned' ? 'success' : 'error' }}" aria-label="{{ ($user->status ?? 'active') == 'Banned' ? 'Unban' : 'Ban' }} {{ $user->name }}"
                                                @if ($user->id === Auth::id()) disabled title="Cannot ban yourself" @endif>
                                                <i class="bi {{ ($user->status ?? 'Active') == 'Banned' ? 'bi-check-circle' : 'bi-x-circle' }}"></i>
                                                {{ ($user->status ?? 'active') == 'Banned' ? 'Unban' : 'Ban' }}
                                                <span class="tooltip">{{ ($user->status ?? 'Active') == 'Banned' ? 'Unban Admin' : 'Ban Admin' }}</span>
                                            </button>
                                        </form>
                                        <form action="{{ route('admin.users.delete', $user->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('Are you sure you want to permanently delete this admin? This action cannot be undone.');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="action-btn error" aria-label="Delete {{ $user->name }}"
                                                ...
                                                <span class="tooltip">Delete Admin</span>
                                            </button>
                                        </form>
                                    </div>
                                </td>
